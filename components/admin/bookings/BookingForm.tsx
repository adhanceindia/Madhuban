...
export function BookingForm() {
  ...
  const [form, setForm] = useState({
    ...
    source: 'manual' as 'website' | 'booking_com' | 'mmt' | 'manual',
  })
  ...
  useEffect(() => {
    fetch('/api/admin/rooms')
      ...
      .then((d) => {
        ...
        if (activeRooms.length > 0) setForm((f) => ({ ...f, room_id: activeRooms[0].id }))
      })
  }, [])
    ...
    <form onSubmit={handleSubmit} className="max-w-[800px] space-y-5">
      <FormCard title="Room & dates">
        ...
        <Field
          ...
          required
          hint={selectedRoom ? `Max ${selectedRoom.capacity} for this room` : undefined}
        >
          <TextInput
            required
            type="number"
        ...
        {calculation && (
          <div className="bg-sage-soft/60 rounded-lg p-4 space-y-1.5 text-[13px]">
            <div className="flex justify-between text-muted-foreground">
              <span>{calculation.nights} nights × {formatINR(selectedRoom?.price_per_night || 0)}</span>
              <span className="font-admin-mono">{formatINR(calculation.subtotal)}</span>
            </div>
            <div className="flex justify-between text-muted-foreground">
              <span>GST (12%)</span>
              <span className="font-admin-mono">{formatINR(calculation.gst)}</span>
            </div>
            <div className="flex justify-between font-semibold text-foreground border-t border-border pt-1.5 mt-1.5">
              <span>Total</span>
              <span className="font-admin-mono">{formatINR(calculation.total)}</span>
            </div>
          </div>
        )}
      </FormCard>
        ...
        <FormRow>
          <Field label="Payment method" required>
            <Select
              ...
              onChange={(e) => setForm({ ...form, payment_method: e.target.value as 'online' | 'at_reception' })}
            >
