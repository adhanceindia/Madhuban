...
export const bookingCreateSchema = z.object({
  ...
  source: z.enum(['website', 'booking_com', 'mmt', 'manual']).default('manual'),
...
export const bookingUpdateSchema = z.object({
  ...
  check_in: z.string().refine((v) => !isNaN(Date.parse(v))).optional(),
  check_out: z.string().refine((v) => !isNaN(Date.parse(v))).optional(),
